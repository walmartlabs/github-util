...
module.exports = {
  ...
  ensureFetched: function(dir, callback) {
    ...
    var fetch = childProcess.exec('git fetch', {cwd: dir, stdio: 'ignore', timeout: 30000}, function(err, stdout, stderr) {
      // TODO : Timeout tracking
      if (err) {
        err = new Error('git.fetch ' + dir + ': ' + err.message);
        err.killed = fetch.killed;
        return callback(err);
      }

      childProcess.exec('git branch -v --no-color | grep -e "^\\*"', {cwd: dir}, function(err, stdout, stderr) {
        if (err) {
          callback(new Error('git.fetch ' + dir + ': ' + err.message));
  ...
  status: function(dir, callback) {
    if (!callback) {
      callback = dir;
      dir = undefined;
    }

    childProcess.exec('git status --porcelain', {cwd: dir}, function(err, stdout) {
      if (err) {
        return callback(new Error('git.status ' + dir + ': ' + err.message));
      }
  ...
  stashes: function(dir, callback) {
    if (!callback) {
      callback = dir;
      dir = undefined;
    }

    childProcess.exec('git stash list', {cwd: dir}, function(err, stdout) {
      if (err) {
        return callback(new Error('git.stash ' + dir + ': ' + err.message));
      }
